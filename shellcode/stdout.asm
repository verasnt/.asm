...
BITS 32
    ...
    mov al, 4                   ; write (syscall 4)
                                ...
                                ; write syscall requires "int fd" in EBX (fd is the file descriptor)
    xor ebx, ebx                ; zero out EBX
    mov bl, 1                   ; stdout file descriptor (fd) is 1

                                ; write syscall requires "const void *buf" in ECX (pointer to the string we want to write)
...
get_string_return:              ; Define a label so we know where to return to our code
    pop ecx                     ; pop the address of the string from the stack (address was pushed onto stack from call instruction below)
                                ...
                                ; write syscall requires "size_t count" in EDX (count is the number of characters in the string we want to write)
